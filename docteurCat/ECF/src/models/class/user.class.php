...
class User extends Database
{
  ...
  public function supprimer($id)
  {
    try {
      ...
      $supp = $this->pdo->prepare('DELETE FROM users WHERE id = :id');
  ...
  //   if(isset($_SESSION['user']) && $_POST['action'] === 'postComment' && isset($_POST['message'])) {
  //     $sql = $this->pdo->prepare("INSERT INTO comment_users (description) VALUES (:description)");
  //     $sql->bindParam(':description', $_POST['message']);
  ...
  //     }
  //   } else {
  //     echo "Erreur: Veuillez vous connecter et remplir tous les champs nécessaires pour ajouter un commentaire.";
  //   }
  // }
  ...
  public function addComment()
  {
    ...
    $checkUser = $this->pdo->prepare("SELECT * FROM users WHERE pseudo = :pseudo");
    $checkUser->bindParam(':pseudo', $_POST['pseudo']);
    $checkUser->execute();
    ...
    if ($user) {
      ...
      $addComment = $this->pdo->prepare("INSERT INTO user_comments (description) VALUES (:description)");

      $addComment->bindParam(':description', $_POST['description']);
      ...
      echo "User not found. Please log in or register before adding a comment.";
    }
  }
}
